...
public class PluginManager {
    ...
    private void loadActivatedPlugins() {
        ...
        List<Plugin> activatedPlugins = pluginRepository.findByStatus("activated");
        int loaded = 0;
        for (Plugin plugin : activatedPlugins) {
            if (!plugin.getIsBundled()) {
                try {
                    activatePlugin(plugin.getPluginId());
                    loaded++;
                } catch (Exception e) {
                    log.error("Failed to activate plugin: {}", plugin.getPluginId(), e);
                }
            }
        }
        log.info("Loaded {} third-party plugins", loaded);
    }
