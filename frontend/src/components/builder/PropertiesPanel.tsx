                  ...
                  onTest={async (config: DataSourceConfig) => {
                    if (config.type === 'api' && config.endpoint) {
                      ...
                      if (!response.ok) {
                        ...
                        errorDetails.push(`Status: ${response.status} ${statusText}`);
                        errorDetails.push(`Endpoint: ${config.endpoint}`);
                        errorDetails.push(`Method: ${config.method || 'GET'}`);

                        if (data?.message) {
                          errorDetails.push(`Message: ${data.message}`);
                        }
                        if (data?.error && data.error !== statusText) {
                          errorDetails.push(`Error: ${data.error}`);
                        }
                        if (data?.path) {
                          errorDetails.push(`Path: ${data.path}`);
                        }
                        if (data?.timestamp) {
                          errorDetails.push(`Time: ${data.timestamp}`);
                        }

                        // Add hint for common errors
                        if (response.status === 403) {
                          errorDetails.push('');
                          errorDetails.push('Hint: 403 Forbidden usually indicates a CORS or authentication issue.');
                          errorDetails.push('Make sure the backend server is running and CORS is configured.');
                        } else if (response.status === 404) {
                          errorDetails.push('');
                          errorDetails.push('Hint: 404 Not Found - check that the endpoint URL is correct.');
                        } else if (response.status === 401) {
                          errorDetails.push('');
                          errorDetails.push('Hint: 401 Unauthorized - authentication required. Add Authorization header.');
                        }
